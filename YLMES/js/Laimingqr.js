    ...
    layui.config({
            ...
            setTimeout(() => {
                ...
                $(this).parent().next().find("dt").each(function () {
                    if (!reg.test(this.innerHTML)) {
                        ...
                        $(this).attr("class", "")
                        arr.push(this.innerHTML.substring(0,4))
                    }
                })
                $(this).parent().next().find("dd").each(function () {
                    arr.forEach((va)=>{
                        if (new RegExp(va).test(this.innerHTML)) {
                            $(this).attr("class", "")
                        ...
                        if (!$(this).attr("class").includes("layui-hide")) {
                            arr1.push(this.innerHTML.substring(0, 4))
                            if (boo) {
       ...
       var s3 =  today2.getFullYear() + "-" + (today2.getMonth()+1) + "-" + today2.getDate();
        ...
        $('#b2c').on('click', function () {
            ...
            if (val == val1) {
                if (!bo) {
                    $.post("/Financial/AccuntingAD", { data: table.cache.myBtn.map(x => { return { name: x.name, name1: x.name1, name2: x.name2, name3: x.name3, name4: x.name4, numBering: x.numBering } }), pzdanh: s2, jzh: $('#jzh2').val() })
                        layer.closeAll()
                        layer.msg("保存成功")
                        table2.reload({
                            url: '/Financial/SeAccuntingListpz'
                            ...
                            , cols: [[
                                ...
                                , {
                                    field: 'name1', templet: function (d) {
                                        var a = JSON.parse(localStorage.select)
        ...
        if (localStorage.select == undefined) {
            select.render({
                ...
                localStorage.select = JSON.stringify(data)
            });
        }
    })